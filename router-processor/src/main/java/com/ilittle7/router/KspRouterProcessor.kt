...
class KspRouterProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        if (!hasNewRawSymbols && !routerManagerGenerated) {
    ...
    private fun generateFinalRouterManager(metadataClasses: List<KSClassDeclaration>) {
        ...
        metadataClasses.forEach { metadata ->
            val anno = metadata.annotations.first { it.shortName.asString() == "GeneratedMetadata" }
        ...
        val moduleName = options["moduleName"] ?: (routerDecls.firstOrNull() ?: globalDecls.firstOrNull())?.packageName?.asString()?.replace(".", "_") ?: "Default"
    ...
    private fun getCustomInterceptors(routerClass: KSClassDeclaration): List<Pair<Int, KSType>> {
        val annotation = routerClass.annotations.find { it.shortName.asString() == "Router" }
        val types = annotation?.arguments?.find { it.name?.asString() == "interceptorArr" }?.value as? List<KSType> ?: emptyList()
        return types.map { (it.declaration as KSClassDeclaration).getPriority() to it }.sortedByDescending { it.first }
    }
