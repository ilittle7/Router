...
### Gradle依赖配置
首先在项目级别的`build.gradle`文件中加入router所在的maven库，并添加router插件的`classpath`：
```groovy
buildscript {
    ext.router_version = '30'
    repositories {
        maven {
            name "maven-releases"
    ...
    dependencies {
        classpath "com.bybutter:router-plugin:$router_version"
    }
}
...
```groovy
dependencies {
	implementation "com.bybutter:router-annotation:$router_version"
	implementation "com.bybutter:router-lib:$router_version"
	kapt "com.bybutter:router-processor:$router_version"
}
```
